...
function renderResult(result) {
  return `
    <div class="obsInfo" role="region">
      <p>
      <strong>${result.comName}</strong>
      <img src="${result.image}" alt="${result.comName}"/>
...
function render() {
  ...
  let appHTML = `
    ...
    <div id="birdForm" role="region"></div>
    <div id="loadingDiv" role="region"></div>
    <div id="results" role="region"></div>
  `;
  ...
  if (RENDERSTATE.formNeedsRender) {
      let searchFormHTML = `
      <form action="#" class="js-search-form" role="form">
  ...
  if (RENDERSTATE.resultsNeedRender) {
      const resultsHTML = `
    <div class="js-search-results" aria-live="assertive" ${(results.length > 0) ? '' : 'hidden'} role="region">
      <div class="pageButtons" role="region">
